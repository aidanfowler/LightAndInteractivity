...
void loop() {
  ...
  for (int level = 0; level < 256; level++) {
    randomFlicker(4,level);
    randomFlicker(21,level);
    randomFlicker(41,level);
    randomFlicker(61,level);
    randomFlicker(81,level);
    randomFlicker(82,level);
    randomFlicker(83,level);
    randomFlicker(104,level);
    Serial.print("DANCE!!! : ");                          // print level
  ...
  for (int level = 255; level >= 0; level--) {
    ...
    randomFlicker(82,level);
    randomFlicker(83,level);
    randomFlicker(104,level);        // set channel 1 (intensity)
...
void randomFlicker(int channel, int level){
  if(random(0,2) == 1){
    myController.setChannel(channel, level);
  }
  else{
    myController.setChannel(channel,0);
  }
}
